...
(function () {
    ...
    Runner.customSounds = {
        BUTTON_PRESS: "custom-sound-press",
        HIT: "custom-sound-hit",
        SCORE: "custom-sound-reached",
    };
    ...
    Runner.prototype = {
        ...
        loadSounds: function () {
            ...
            var resourceTemplate = resourceEl.content;
            ...
            // Load custom audio in background.
            var customEl = document.getElementById('custom-audio-resources');
            if (customEl) {
                var customTemplate = customEl.content;
                var customCount = 0;
                var customTotal = Object.keys(Runner.customSounds).length;
                var self = this;
                for (var csound in Runner.customSounds) {
                    var cSrc = customTemplate.getElementById(
                        Runner.customSounds[csound]
                    ).src;
                    cSrc = cSrc.substr(cSrc.indexOf(",") + 1);
                    var cBuffer = decodeBase64ToArrayBuffer(cSrc);
                    this.audioContext.decodeAudioData(
                        cBuffer,
                        (function (index) {
                            return function (audioData) {
                                self.customSoundFx[index] = audioData;
                                customCount++;
                                if (customCount >= customTotal) {
                                    self.customSoundsReady = true;
                                }
                            };
                        })(csound)
                    );
        ...
        onKeyDown: function (e) {
            ...
            if (e.keyCode === 80 && !this.crashed && this.activated) {
                if (this.playing) {
                    this.stop();
                } else if (this.paused) {
                    if (typeof hidePauseOverlay === 'function') hidePauseOverlay();
        ...
        setArcadeModeContainerScale() {
            ...
            var scale;
            if (window.__forceRes === 1) {
                scale = 1;
            } else if (window.__forceRes === 2) {
                scale = 2;
            } else {
        ...
        playSound: function (soundBuffer) {
            ...
            var buf = soundBuffer;
            if (window.__customAudio && this.customSoundsReady) {
                // Find which default sound this is and swap for custom
                for (var key in this.soundFx) {
                    if (this.soundFx[key] === soundBuffer && this.customSoundFx[key]) {
                        buf = this.customSoundFx[key];
                        break;
                    }
                }
            }
        ...
        (Obstacle.prototype = {
            ...
            init: function (speed) {
                ...
                if (Array.isArray(this.typeConfig.yPos)) {
                    ...
                    this.yPos =
                        yPosConfig[getRandomNum(0, yPosConfig.length - 1)];
                } else {
    ...
    Trex.prototype = {
        ...
        startJump: function (speed) {
            if (!this.jumping) {
                ...
                this.jumpVelocity =
                    this.config.INIITAL_JUMP_VELOCITY - speed / 10;
                this.jumping = true;
                this.reachedMinHeight = false;
                this.speedDrop = false;
            }
